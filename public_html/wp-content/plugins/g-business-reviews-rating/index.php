...
class google_business_reviews_rating
{
	...
	public static function activate()
	{
		...
		if (is_bool(get_option(__CLASS__ . '_place_id')))
		{
			...
			update_option(__CLASS__ . '_force', NULL, 'yes');
...
';
	...
	public function admin_ajax()
	{
		...
		$sort = (isset($post['sort']) && preg_match('/^reviews$/i', $post['section']) && !preg_match('/^relevance(?:[_-]desc)?$/', $post['sort'])) ? preg_replace('/[^\w_-]/', '', strtolower($post['sort'])) : NULL;
...
';
	...
	public function set_data($force = NULL, $api_key = NULL, $place_id = NULL)
	{
		...
		{
			$force_check = get_option(__CLASS__ . '_force', NULL);
			
			if (is_string($force_check) && preg_match('#^(\d+(?:\.\d+)?)/0$#', $force_check, $m))
			{
				$force = ((time() - intval($m[1])) < 10);
				update_option(__CLASS__ . '_force', $m[1] . '/1', 'yes');
			}
		}
...
';
	...
	private function reviews_filter($filters = NULL, $atts = NULL)
	{
		...
		$sort = ($id == NULL && array_key_exists('sort', $filters) && is_string($filters['sort'])) ? preg_replace('/[^\w_-]/', '', $filters['sort']) : get_option(__CLASS__ . '_review_sort', NULL);
	...
	public function sanitize_api_key($api_key)
	{
		...
		if (get_option(__CLASS__ . '_api_key') != $api_key)
		{
			...
			if ($api_key != NULL)
			{
				update_option(__CLASS__ . '_force', time() . '/0', 'yes');
			}
		}
    ...
    <thead>
        <tr>
            <th class="id number'  . (($this->review_sort != NULL && preg_match('/^ids?(?:_(?:asc|desc))?$/i', $this->review_sort)) ? ' sorted' . ((!is_bool($this->review_sort_asc) || is_bool($this->review_sort_asc) && $this->review_sort_asc) ? ' asc' : ' desc') : '') . '" title="ID"><a href="#reviews-table" class="sort" data-field="id"><span>ID</span> <span class="sorting-indicator"></span></a></th>
            <th class="submitted date'  . (($this->review_sort != NULL && preg_match('/^(?:date|submitted|time)(?:_(?:asc|desc))?$/i', $this->review_sort)) ? ' sorted' . ((!is_bool($this->review_sort_asc) || is_bool($this->review_sort_asc) && $this->review_sort_asc) ? ' asc' : ' desc') : '') . '" title="Submitted"><a href="#reviews-table" class="sort" data-field="time">Submitted</span> <span class="sorting-indicator"></span></a></th>
            <th class="author'  . (($this->review_sort != NULL && preg_match('/^(?:author(?:[_-]name)?|name)(?:_(?:asc|desc))?$/i', $this->review_sort)) ? ' sorted' . ((!is_bool($this->review_sort_asc) || is_bool($this->review_sort_asc) && $this->review_sort_asc) ? ' asc' : ' desc') : '') . '" title="Author"><a href="#reviews-table" class="sort" data-field="author_name"><span>Author</span> <span class="sorting-indicator"></span></a></th>
            <th class="rating'  . (($this->review_sort != NULL && preg_match('/^ratings?(?:_(?:asc|desc))?$/i', $this->review_sort)) ? ' sorted' . ((!is_bool($this->review_sort_asc) || is_bool($this->review_sort_asc) && $this->review_sort_asc) ? ' asc' : ' desc') : '') . '" title="Rating"><a href="#reviews-table" class="sort" data-field="rating"><span>Rating</span> <span class="sorting-indicator"></span></a></th>
            <th class="text'  . (($this->review_sort != NULL && preg_match('/^(?:review(?:[_-]text)?|text)(?:_(?:asc|desc))?$/', $this->review_sort)) ? ' sorted' . ((!is_bool($this->review_sort_asc) || is_bool($this->review_sort_asc) && $this->review_sort_asc) ? ' asc' : ' desc') : '') . '" title="Text"><a href="#reviews-table" class="sort" data-field="text"><span>Text</span> <span class="sorting-indicator"></span></a></th>
            <th class="language'  . (($this->review_sort != NULL && preg_match('/^languages?(?:_(?:asc|desc))?$/i', $this->review_sort)) ? ' sorted' . ((!is_bool($this->review_sort_asc) || is_bool($this->review_sort_asc) && $this->review_sort_asc) ? ' asc' : ' desc') : '') . '" title="Language"><a href="#reviews-table" class="sort" data-field="language">Language</span> <span class="sorting-indicator"></span></a></th>
            <th class="retrieved date'  . (($this->review_sort != NULL && preg_match('/^retrieved(?:_(?:asc|desc))?$/i', $this->review_sort)) ? ' sorted' . ((!is_bool($this->review_sort_asc) || is_bool($this->review_sort_asc) && $this->review_sort_asc) ? ' asc' : ' desc') : '') . '" title="Retrieved"><a href="#reviews-table" class="sort" data-field="retrieved">Retrieved</span> <span class="sorting-indicator"></span></a></th>
';
			if ($show_place_id)
			{
				$html .= '            <th class="place-id'  . (($this->review_sort != NULL && preg_match('/^place_ids?(?:_(?:asc|desc))?$/i', $this->review_sort)) ? ' sorted' . ((!is_bool($this->review_sort_asc) || is_bool($this->review_sort_asc) && $this->review_sort_asc) ? ' asc' : ' desc') : '') . '" title="Place ID"><a href="#reviews-table" class="sort" data-field="place_id">Place ID</span> <span class="sorting-indicator"></span></a></th>
';
			}
...
';		
			foreach ($this->reviews as $key => $a)
			{
				$html .= '        <tr id="' . esc_attr(preg_replace('/[^0-9a-z-]/', '-', $key)) . '" class="review ' . esc_attr('rating-' . $a['rating']) . esc_attr((!$a['status']) ? ' inactive' : '') . esc_attr((array_key_exists('time_estimate', $a) && $a['time_estimate']) ? ' estimate' : '') . '" data-id="' . esc_attr($a['id']) . '" data-order="' . esc_attr($a['order']) . '">
            <td class="id number">' . esc_html($a['id']) . ' <a href="' . esc_attr('#' . preg_replace('/[^0-9a-z-]/', '-', $key)) . '" class="show-hide" title="' . (($a['status']) ? 'Hide' : 'Show') . '">' . (($a['status']) ? '<span class="dashicons dashicons-visibility"></span>' : '<span class="dashicons dashicons-hidden"></span>') . '</a>' . ((array_key_exists('time_estimate', $a) && $a['time_estimate']) ? '<a href="' . esc_attr('#' . preg_replace('/[^0-9a-z-]/', '-', $key)) . '" class="remove" title="' . esc_attr__('Remove', 'g-business-reviews-rating') . '"><span class="dashicons dashicons-no"></span></a>' : '') . '</td>
            <td class="submitted date"><span class="date' . ((array_key_exists('time_estimate', $a) && $a['time_estimate']) ? ' date-edit' : '') . '"><span class="value">' . ((array_key_exists('time_estimate', $a) && $a['time_estimate']) ? esc_html(date("Y/m/d", $a['time'])) . '</span> <span class="dashicons dashicons-arrow-down"></span>' : esc_html(date("Y/m/d H:i", $a['time']))) . '</span>' . ((array_key_exists('time_estimate', $a) && $a['time_estimate']) ? '<input type="date" id="' . esc_attr('submitted-' . preg_replace('/[^0-9a-z-]/', '-', $key)) . '" class="time-estimate" name="submitted[]" value="' . esc_attr(date("Y-m-d", $a['time'])) . '" max="' . esc_attr(date("Y-m-d")) . '">' : '') . '</td>
            <td class="author">
...
';
	...
	public function wp_display($atts = NULL, $content = NULL, $shortcode = NULL)
	{
		...
		$stars = (is_null($stars) || is_bool($stars) && $stars || is_string($stars) && preg_match('/^(?:t(?:rue)?|y(?:es)?|1|on|show|svg|vector)$/i', $stars)) ? (is_bool($summary) || is_array($summary) && in_array('stars', $summary)) : ((is_string($stars) && preg_match('/(#(?:[0-9a-f]{2}){2,4}|#[0-9a-f]{3}|(?:rgba?|hsla?)\((?:\d+%?(?:deg|rad|grad|turn)?(?:,|\s)+){2,3}[\s\/]*[\d\.]+%?\))/i', $stars)) ? $stars : ((is_string($stars) && preg_match('/^(?:html|css)$/i', $stars)) ? strtolower($stars) : FALSE));
		...
		$count = (is_null($count) || is_bool($count) && $count || is_string($count) && preg_match('/^(?:t(?:rue)?|y(?:es)?|1|on|show)$/i', $count) || is_array($summary) && in_array('count', $summary));
